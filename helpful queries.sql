...
DROP FUNCTION IF EXISTS status.site_list;
CREATE OR REPLACE FUNCTION status.site_list (inverters text[])
RETURNS TABLE (
  host_rcpn text,
  device_id text,
  device_type text,
...
BEGIN
 ...
 SELECT ds.host_rcpn, ds.device_id, ds.device_type, ds.timestamp_utc as last_update_utc,
    ...
    FROM unnest( inverters ) as inverter(device_id)
...
select * from status.site_list( ARRAY[('000100072DAB'), ('0001000720D0')]  );
select * from status.site_list( select host_rcpn from status.device_shadow order by timestamp_utc desc limit 10  );
