...
WITH data AS
    ( SELECT device_id, host_rcpn, st, device_type, timestamp_utc
     FROM status.device_shadow ds
      JOIN unnest(ARRAY[
       '000100072DAB', '000100071818', '000100072021', '0001000712E6', '000100072E46', '0001000705DB', '000100070B1E'
        ]) host
     ON host_rcpn = host
     )
...
WHERE (e.created_timestamp_utc=
       ...
       OR e.created_timestamp_utc IS NULL) -- latest row from ess_device_info
AND (n.created_timestamp_utc=
         (SELECT max(created_timestamp_utc)
          FROM status.nameplate
          WHERE device_id=d.device_id)
     OR n.created_timestamp_utc IS NULL)  -- latest row from nameplate
